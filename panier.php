...
<?php 
 ...
 // permet d afficher les produit ajouter au panier
...
// si il clic sur le potton acheter
if (isset($_POST['pai'])) {

// il recuper les info des produit de client 
$statement = $pdo->prepare("SELECT * FROM table_panier WHERE idper=?");
$statement->execute(array($_SESSION['client']['id_c']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC); 

// pour chaque produit selectionne il va ajouter a table commande                          
foreach ($result as $row) { 
    $idpanier=$row['id'];
...
<!-- ce php permet de verfie cest le client a des produit dans la table commande -->
<!-- si oui il va afficher le msg -->
<?php
